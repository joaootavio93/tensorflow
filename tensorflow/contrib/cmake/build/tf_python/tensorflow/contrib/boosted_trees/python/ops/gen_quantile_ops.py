...
def bucketize_with_input_boundaries(input, boundaries, name=None):
  ...
  if _context.in_graph_mode():
    ...
    boundaries = _tensor.convert_to_eager_tensor(boundaries, _dtypes.float32)
    _inputs_flat = [input, boundaries]
    _attrs = ("T", _attr_T)
    _result = _execute.execute("BucketizeWithInputBoundaries", 1,
...
def create_quantile_accumulator(quantile_accumulator_handle, stamp_token, epsilon, num_quantiles, container="", shared_name="", max_elements=1099511627776, name=None):
  ...
  if _context.in_graph_mode():
    ...
    quantile_accumulator_handle = _tensor.convert_to_eager_tensor(quantile_accumulator_handle, _dtypes.resource)
    stamp_token = _tensor.convert_to_eager_tensor(stamp_token, _dtypes.int64)
    _inputs_flat = [quantile_accumulator_handle, stamp_token]
    _attrs = ("container", container, "shared_name", shared_name,
              "max_elements", max_elements, "epsilon", epsilon,
              "num_quantiles", num_quantiles)
    _result = _execute.execute("CreateQuantileAccumulator", 0,
                               inputs=_inputs_flat, attrs=_attrs, name=name)
  return _result
...
def make_quantile_summaries(dense_float_features, sparse_float_feature_indices, sparse_float_feature_values, sparse_float_feature_shapes, example_weights, epsilon, name=None):
  ...
  if _context.in_graph_mode():
    ...
    dense_float_features = _tensor.convert_n_to_eager_tensor(dense_float_features, _dtypes.float32)
    sparse_float_feature_indices = _tensor.convert_n_to_eager_tensor(sparse_float_feature_indices, _dtypes.int64)
    sparse_float_feature_values = _tensor.convert_n_to_eager_tensor(sparse_float_feature_values, _dtypes.float32)
    sparse_float_feature_shapes = _tensor.convert_n_to_eager_tensor(sparse_float_feature_shapes, _dtypes.int64)
    example_weights = _tensor.convert_to_eager_tensor(example_weights, _dtypes.float32)
    _inputs_flat = list(dense_float_features) + list(sparse_float_feature_indices) + list(sparse_float_feature_values) + list(sparse_float_feature_shapes) + [example_weights]
    _attrs = ("num_dense_features", _attr_num_dense_features,
              "num_sparse_features", _attr_num_sparse_features, "epsilon",
              epsilon)
    _result = _execute.execute("MakeQuantileSummaries",
...
def quantile_accumulator_add_summaries(quantile_accumulator_handles, stamp_token, summaries, name=None):
  ...
  if _context.in_graph_mode():
    ...
    quantile_accumulator_handles = _tensor.convert_n_to_eager_tensor(quantile_accumulator_handles, _dtypes.resource)
    stamp_token = _tensor.convert_to_eager_tensor(stamp_token, _dtypes.int64)
    summaries = _tensor.convert_n_to_eager_tensor(summaries, _dtypes.string)
    ...
    _result = _execute.execute("QuantileAccumulatorAddSummaries", 0,
                               inputs=_inputs_flat, attrs=_attrs, name=name)
  return _result
...
def quantile_accumulator_deserialize(quantile_accumulator_handle, stamp_token, stream_state, are_buckets_ready, buckets, name=None):
  ...
  if _context.in_graph_mode():
    ...
    quantile_accumulator_handle = _tensor.convert_to_eager_tensor(quantile_accumulator_handle, _dtypes.resource)
    stamp_token = _tensor.convert_to_eager_tensor(stamp_token, _dtypes.int64)
    stream_state = _tensor.convert_to_eager_tensor(stream_state, _dtypes.string)
    are_buckets_ready = _tensor.convert_to_eager_tensor(are_buckets_ready, _dtypes.bool)
    buckets = _tensor.convert_to_eager_tensor(buckets, _dtypes.float32)
    _inputs_flat = [quantile_accumulator_handle, stamp_token, stream_state, are_buckets_ready, buckets]
    _attrs = None
    _result = _execute.execute("QuantileAccumulatorDeserialize", 0,
                               inputs=_inputs_flat, attrs=_attrs, name=name)
  return _result
...
def quantile_accumulator_flush(quantile_accumulator_handle, stamp_token, next_stamp_token, name=None):
  ...
  if _context.in_graph_mode():
    ...
    quantile_accumulator_handle = _tensor.convert_to_eager_tensor(quantile_accumulator_handle, _dtypes.resource)
    stamp_token = _tensor.convert_to_eager_tensor(stamp_token, _dtypes.int64)
    next_stamp_token = _tensor.convert_to_eager_tensor(next_stamp_token, _dtypes.int64)
    _inputs_flat = [quantile_accumulator_handle, stamp_token, next_stamp_token]
    _attrs = None
    _result = _execute.execute("QuantileAccumulatorFlush", 0,
                               inputs=_inputs_flat, attrs=_attrs, name=name)
  return _result
...
def quantile_accumulator_flush_summary(quantile_accumulator_handle, stamp_token, next_stamp_token, name=None):
  ...
  if _context.in_graph_mode():
    ...
    _inputs_flat = [quantile_accumulator_handle, stamp_token, next_stamp_token]
    _attrs = None
    _result = _execute.execute("QuantileAccumulatorFlushSummary", 1,
                               inputs=_inputs_flat, attrs=_attrs, name=name)
  _result = _execute.record_gradient(
...
def quantile_accumulator_get_buckets(quantile_accumulator_handles, stamp_token, name=None):
  ...
  if _context.in_graph_mode():
    ...
    _inputs_flat = list(quantile_accumulator_handles) + [stamp_token]
    _attrs = ("num_resource_handles", _attr_num_resource_handles)
    _result = _execute.execute("QuantileAccumulatorGetBuckets",
...
def quantile_accumulator_is_initialized(quantile_accumulator_handle, name=None):
  ...
  if _context.in_graph_mode():
    ...
    _inputs_flat = [quantile_accumulator_handle]
    _attrs = None
    _result = _execute.execute("QuantileAccumulatorIsInitialized", 1,
                               inputs=_inputs_flat, attrs=_attrs, name=name)
  _result = _execute.record_gradient(
...
def quantile_accumulator_serialize(quantile_accumulator_handle, name=None):
  ...
  if _context.in_graph_mode():
    ...
    _inputs_flat = [quantile_accumulator_handle]
    _attrs = None
    _result = _execute.execute("QuantileAccumulatorSerialize", 4,
                               inputs=_inputs_flat, attrs=_attrs, name=name)
  _result = _execute.record_gradient(
...
def quantile_buckets(dense_float_features, sparse_float_feature_indices, sparse_float_feature_values, sparse_float_feature_shapes, example_weights, dense_config, sparse_config, name=None):
  ...
  if _context.in_graph_mode():
    ...
    _result = _execute.execute("QuantileBuckets", _attr_num_dense_features +
                               _attr_num_sparse_features, inputs=_inputs_flat,
                               attrs=_attrs, name=name)
  _result = _execute.record_gradient(
...
def quantile_stream_resource_handle_op(container="", shared_name="", name=None):
  ...
  if _context.in_graph_mode():
    ...
    _inputs_flat = []
    _attrs = ("container", container, "shared_name", shared_name)
    _result = _execute.execute("QuantileStreamResourceHandleOp", 1,
                               inputs=_inputs_flat, attrs=_attrs, name=name)
  _result = _execute.record_gradient(
...
def quantiles(dense_values, sparse_values, dense_buckets, sparse_buckets, name=None):
  ...
  if _context.in_graph_mode():
    ...
    dense_values = _tensor.convert_n_to_eager_tensor(dense_values, _dtypes.float32)
    sparse_values = _tensor.convert_n_to_eager_tensor(sparse_values, _dtypes.float32)
    dense_buckets = _tensor.convert_n_to_eager_tensor(dense_buckets, _dtypes.float32)
    sparse_buckets = _tensor.convert_n_to_eager_tensor(sparse_buckets, _dtypes.float32)
    _inputs_flat = list(dense_values) + list(sparse_values) + list(dense_buckets) + list(sparse_buckets)
    _attrs = ("num_dense_features", _attr_num_dense_features,
              "num_sparse_features", _attr_num_sparse_features)
    _result = _execute.execute("Quantiles", _attr_num_dense_features +
                               _attr_num_sparse_features, inputs=_inputs_flat,
                               attrs=_attrs, name=name)
  _result = _execute.record_gradient(
