...
def center_tree_ensemble_bias(tree_ensemble_handle, stamp_token, next_stamp_token, delta_updates, learner_config, centering_epsilon=0.01, name=None):
  ...
  if _context.in_graph_mode():
    ...
    delta_updates = _tensor.convert_to_eager_tensor(delta_updates, _dtypes.float32)
    _inputs_flat = [tree_ensemble_handle, stamp_token, next_stamp_token, delta_updates]
    _attrs = ("learner_config", learner_config, "centering_epsilon",
              centering_epsilon)
    _result = _execute.execute("CenterTreeEnsembleBias", 1,
...
def grow_tree_ensemble(tree_ensemble_handle, stamp_token, next_stamp_token, learning_rate, dropout_seed, partition_ids, gains, splits, learner_config, center_bias, name=None):
  ...
  if _context.in_graph_mode():
    ...
    stamp_token = _tensor.convert_to_eager_tensor(stamp_token, _dtypes.int64)
    next_stamp_token = _tensor.convert_to_eager_tensor(next_stamp_token, _dtypes.int64)
    learning_rate = _tensor.convert_to_eager_tensor(learning_rate, _dtypes.float32)
    dropout_seed = _tensor.convert_to_eager_tensor(dropout_seed, _dtypes.int64)
    partition_ids = _tensor.convert_n_to_eager_tensor(partition_ids, _dtypes.int32)
    gains = _tensor.convert_n_to_eager_tensor(gains, _dtypes.float32)
    splits = _tensor.convert_n_to_eager_tensor(splits, _dtypes.string)
    ...
    _result = _execute.execute("GrowTreeEnsemble", 0, inputs=_inputs_flat,
...
def tree_ensemble_stats(tree_ensemble_handle, stamp_token, name=None):
  ...
  if _context.in_graph_mode():
    ...
    _inputs_flat = [tree_ensemble_handle, stamp_token]
    _attrs = None
    _result = _execute.execute("TreeEnsembleStats", 6, inputs=_inputs_flat,
                               attrs=_attrs, name=name)
  _result = _execute.record_gradient(
