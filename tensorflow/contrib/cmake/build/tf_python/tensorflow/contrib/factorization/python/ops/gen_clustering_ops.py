...
def kmeans_plus_plus_initialization(points, num_to_sample, seed, num_retries_per_sample, name=None):
  ...
  if _context.in_graph_mode():
    ...
    points = _tensor.convert_to_eager_tensor(points, _dtypes.float32)
    num_to_sample = _tensor.convert_to_eager_tensor(num_to_sample, _dtypes.int64)
    seed = _tensor.convert_to_eager_tensor(seed, _dtypes.int64)
    num_retries_per_sample = _tensor.convert_to_eager_tensor(num_retries_per_sample, _dtypes.int64)
    _inputs_flat = [points, num_to_sample, seed, num_retries_per_sample]
    _attrs = None
    _result = _execute.execute("KmeansPlusPlusInitialization", 1,
...
def nearest_neighbors(points, centers, k, name=None):
  ...
  if _context.in_graph_mode():
    ...
    points = _tensor.convert_to_eager_tensor(points, _dtypes.float32)
    centers = _tensor.convert_to_eager_tensor(centers, _dtypes.float32)
    k = _tensor.convert_to_eager_tensor(k, _dtypes.int64)
    _inputs_flat = [points, centers, k]
    _attrs = None
    _result = _execute.execute("NearestNeighbors", 2, inputs=_inputs_flat,
