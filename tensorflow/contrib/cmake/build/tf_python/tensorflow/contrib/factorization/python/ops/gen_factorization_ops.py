...
def masked_matmul(a, b, mask_indices, transpose_a, transpose_b, name=None):
  ...
  if _context.in_graph_mode():
    ...
    a = _tensor.convert_to_eager_tensor(a, _dtypes.float32)
    b = _tensor.convert_to_eager_tensor(b, _dtypes.float32)
    mask_indices = _tensor.convert_to_eager_tensor(mask_indices, _dtypes.int64)
    transpose_a = _tensor.convert_to_eager_tensor(transpose_a, _dtypes.bool)
    transpose_b = _tensor.convert_to_eager_tensor(transpose_b, _dtypes.bool)
    _inputs_flat = [a, b, mask_indices, transpose_a, transpose_b]
    _attrs = None
    _result = _execute.execute("MaskedMatmul", 1, inputs=_inputs_flat,
...
def wals_compute_partial_lhs_and_rhs(factors, factor_weights, unobserved_weights, input_weights, input_indices, input_values, input_block_size, input_is_transpose, name=None):
  ...
  if _context.in_graph_mode():
    ...
    factors = _tensor.convert_to_eager_tensor(factors, _dtypes.float32)
    factor_weights = _tensor.convert_to_eager_tensor(factor_weights, _dtypes.float32)
    unobserved_weights = _tensor.convert_to_eager_tensor(unobserved_weights, _dtypes.float32)
    ...
    input_values = _tensor.convert_to_eager_tensor(input_values, _dtypes.float32)
    input_block_size = _tensor.convert_to_eager_tensor(input_block_size, _dtypes.int64)
    input_is_transpose = _tensor.convert_to_eager_tensor(input_is_transpose, _dtypes.bool)
    _inputs_flat = [factors, factor_weights, unobserved_weights, input_weights, input_indices, input_values, input_block_size, input_is_transpose]
    _attrs = None
    _result = _execute.execute("WALSComputePartialLhsAndRhs", 2,
