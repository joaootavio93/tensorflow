...
def generate_vocab_remapping(new_vocab_file, old_vocab_file, new_vocab_offset, num_new_vocab, name=None):
  ...
  if _context.in_graph_mode():
    ...
    new_vocab_file = _tensor.convert_to_eager_tensor(new_vocab_file, _dtypes.string)
    old_vocab_file = _tensor.convert_to_eager_tensor(old_vocab_file, _dtypes.string)
    _inputs_flat = [new_vocab_file, old_vocab_file]
    _attrs = ("new_vocab_offset", new_vocab_offset, "num_new_vocab",
              num_new_vocab)
    _result = _execute.execute("GenerateVocabRemapping", 2,
...
def load_and_remap_matrix(ckpt_path, old_tensor_name, row_remapping, col_remapping, initializing_values, num_rows, num_cols, max_rows_in_memory=-1, name=None):
  ...
  * A value of -1 in either of the remappings signifies a "missing" entry. In that
  ...
  if _context.in_graph_mode():
    ...
    ckpt_path = _tensor.convert_to_eager_tensor(ckpt_path, _dtypes.string)
    old_tensor_name = _tensor.convert_to_eager_tensor(old_tensor_name, _dtypes.string)
    row_remapping = _tensor.convert_to_eager_tensor(row_remapping, _dtypes.int64)
    col_remapping = _tensor.convert_to_eager_tensor(col_remapping, _dtypes.int64)
    initializing_values = _tensor.convert_to_eager_tensor(initializing_values, _dtypes.float32)
    _inputs_flat = [ckpt_path, old_tensor_name, row_remapping, col_remapping, initializing_values]
    _attrs = ("num_rows", num_rows, "num_cols", num_cols,
              "max_rows_in_memory", max_rows_in_memory)
    _result = _execute.execute("LoadAndRemapMatrix", 1, inputs=_inputs_flat,
