...
def bipartite_match(distance_mat, num_valid_rows, top_k=-1, name=None):
  ...
  if _context.in_graph_mode():
    ...
    distance_mat = _tensor.convert_to_eager_tensor(distance_mat, _dtypes.float32)
    num_valid_rows = _tensor.convert_to_eager_tensor(num_valid_rows, _dtypes.float32)
    _inputs_flat = [distance_mat, num_valid_rows]
    _attrs = ("top_k", top_k)
    _result = _execute.execute("BipartiteMatch", 2, inputs=_inputs_flat,
...
def image_projective_transform(images, transforms, interpolation, name=None):
  ...
  if _context.in_graph_mode():
    ...
    _attr_dtype, (images,) = _execute.args_to_matching_eager([images])
    _attr_dtype = _attr_dtype.as_datatype_enum
    transforms = _tensor.convert_to_eager_tensor(transforms, _dtypes.float32)
    _inputs_flat = [images, transforms]
    _attrs = ("dtype", _attr_dtype, "interpolation", interpolation)
    _result = _execute.execute("ImageProjectiveTransform", 1,
