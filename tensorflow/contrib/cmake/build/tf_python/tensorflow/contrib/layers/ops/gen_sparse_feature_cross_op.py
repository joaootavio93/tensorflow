...
def sparse_feature_cross(indices, values, shapes, dense, hashed_output, num_buckets, out_type, internal_type, name=None):
  ...
  if _context.in_graph_mode():
    ...
    _attr_dense_types, dense = _execute.convert_to_mixed_eager_tensors(dense)
    ...
    _attrs = ("N", _attr_N, "hashed_output", hashed_output, "num_buckets",
              ...
              internal_type)
    _result = _execute.execute("SparseFeatureCross", 3, inputs=_inputs_flat,
...
def sparse_feature_cross_v2(indices, values, shapes, dense, hashed_output, num_buckets, hash_key, out_type, internal_type, name=None):
  ...
  if _context.in_graph_mode():
    ...
    _result = _execute.execute("SparseFeatureCrossV2", 3, inputs=_inputs_flat,
                               attrs=_attrs, name=name)
  _result = _execute.record_gradient(
