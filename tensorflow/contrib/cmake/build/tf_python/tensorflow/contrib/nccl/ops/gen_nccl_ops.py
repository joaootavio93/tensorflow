...
def nccl_all_reduce(input, reduction, num_devices, shared_name, name=None):
  ...
  if _context.in_graph_mode():
    ...
    _result = _execute.execute("NcclAllReduce", 1, inputs=_inputs_flat,
...
def nccl_broadcast_recv(shape, T, num_devices, shared_name, name=None):
  ...
  if _context.in_graph_mode():
    ...
    _result = _execute.execute("NcclBroadcastRecv", 1, inputs=_inputs_flat,
                               attrs=_attrs, name=name)
  _result = _execute.record_gradient(
...
def nccl_broadcast_send(input, num_devices, shared_name, name=None):
  ...
  if _context.in_graph_mode():
    ...
    _attrs = ("T", _attr_T, "num_devices", num_devices, "shared_name",
              shared_name)
    _result = _execute.execute("NcclBroadcastSend", 0, inputs=_inputs_flat,
                               attrs=_attrs, name=name)
  return _result
