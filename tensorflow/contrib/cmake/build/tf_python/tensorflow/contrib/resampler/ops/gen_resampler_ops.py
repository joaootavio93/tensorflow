...
def resampler(data, warp, name=None):
  ...
  if _context.in_graph_mode():
    ...
    _attr_T, _inputs_T = _execute.args_to_matching_eager([data, warp])
    (data, warp) = _inputs_T
    _attr_T = _attr_T.as_datatype_enum
    _inputs_flat = [data, warp]
    _attrs = ("T", _attr_T)
    _result = _execute.execute("Resampler", 1, inputs=_inputs_flat,
...
def resampler_grad(data, warp, grad_output, name=None):
  ...
  if _context.in_graph_mode():
    ...
    _attr_T, _inputs_T = _execute.args_to_matching_eager([data, warp, grad_output])
    (data, warp, grad_output) = _inputs_T
    _attr_T = _attr_T.as_datatype_enum
    _inputs_flat = [data, warp, grad_output]
    _attrs = ("T", _attr_T)
    _result = _execute.execute("ResamplerGrad", 2, inputs=_inputs_flat,
                               attrs=_attrs, name=name)
  _result = _execute.record_gradient(
