...
def reinterpret_string_to_float(input_data, name=None):
  ...
  if _context.in_graph_mode():
    ...
    input_data = _tensor.convert_to_eager_tensor(input_data, _dtypes.string)
    _inputs_flat = [input_data]
    _attrs = None
    _result = _execute.execute("ReinterpretStringToFloat", 1,
...
def scatter_add_ndim(input, indices, deltas, name=None):
  ...
  if _context.in_graph_mode():
    ...
    raise RuntimeError(
        "scatter_add_ndim op does not support eager execution.")
  return _result
