...
def skip_gram_generate_candidates(input_tensor, min_skips, max_skips, start, limit, emit_self_as_target, seed=0, seed2=0, name=None):
  ...
  if _context.in_graph_mode():
    ...
    min_skips = _tensor.convert_to_eager_tensor(min_skips, _dtypes.int32)
    max_skips = _tensor.convert_to_eager_tensor(max_skips, _dtypes.int32)
    start = _tensor.convert_to_eager_tensor(start, _dtypes.int32)
    limit = _tensor.convert_to_eager_tensor(limit, _dtypes.int32)
    emit_self_as_target = _tensor.convert_to_eager_tensor(emit_self_as_target, _dtypes.bool)
    _inputs_flat = [input_tensor, min_skips, max_skips, start, limit, emit_self_as_target]
    _attrs = ("T", _attr_T, "seed", seed, "seed2", seed2)
    _result = _execute.execute("SkipGramGenerateCandidates", 2,
