...
def decode_csv(records, record_defaults, field_delim=",", use_quote_delim=True, name=None):
  ...
  if _context.in_graph_mode():
    _, _, _op = _op_def_lib._apply_op_helper(
        ...
        field_delim=field_delim, use_quote_delim=use_quote_delim, name=name)
    _result = _op.outputs[:]
    ...
    _attrs = ("OUT_TYPE", _op.get_attr("OUT_TYPE"), "field_delim",
              ...
              _op.get_attr("use_quote_delim"))
  else:
    _attr_OUT_TYPE, record_defaults = _execute.convert_to_mixed_eager_tensors(record_defaults)
    _attr_OUT_TYPE = [_t.as_datatype_enum for _t in _attr_OUT_TYPE]
    records = _tensor.convert_to_eager_tensor(records, _dtypes.string)
    _inputs_flat = [records] + list(record_defaults)
    _attrs = ("OUT_TYPE", _attr_OUT_TYPE, "field_delim", field_delim,
              "use_quote_delim", use_quote_delim)
    _result = _execute.execute("DecodeCSV", len(record_defaults),
...
def decode_json_example(json_examples, name=None):
  ...
  if _context.in_graph_mode():
    ...
    json_examples = _tensor.convert_to_eager_tensor(json_examples, _dtypes.string)
    _inputs_flat = [json_examples]
    _attrs = None
    _result = _execute.execute("DecodeJSONExample", 1, inputs=_inputs_flat,
...
def decode_raw(bytes, out_type, little_endian=True, name=None):
  ...
  if _context.in_graph_mode():
    ...
    bytes = _tensor.convert_to_eager_tensor(bytes, _dtypes.string)
    _inputs_flat = [bytes]
    _attrs = ("out_type", out_type, "little_endian", little_endian)
    _result = _execute.execute("DecodeRaw", 1, inputs=_inputs_flat,
                               attrs=_attrs, name=name)
  _result = _execute.record_gradient(
...
def _parse_example(serialized, names, sparse_keys, dense_keys, dense_defaults, sparse_types, dense_shapes, name=None):
  ...
  if _context.in_graph_mode():
    ...
    _attr_Tdense, dense_defaults = _execute.convert_to_mixed_eager_tensors(dense_defaults)
    _attr_Tdense = [_t.as_datatype_enum for _t in _attr_Tdense]
    serialized = _tensor.convert_to_eager_tensor(serialized, _dtypes.string)
    names = _tensor.convert_to_eager_tensor(names, _dtypes.string)
    sparse_keys = _tensor.convert_n_to_eager_tensor(sparse_keys, _dtypes.string)
    dense_keys = _tensor.convert_n_to_eager_tensor(dense_keys, _dtypes.string)
    _inputs_flat = [serialized, names] + list(sparse_keys) + list(dense_keys) + list(dense_defaults)
    _attrs = ("Nsparse", _attr_Nsparse, "Ndense", _attr_Ndense,
              "sparse_types", sparse_types, "Tdense", _attr_Tdense,
              "dense_shapes", dense_shapes)
    _result = _execute.execute("ParseExample", _attr_Nsparse +
...
def _parse_single_sequence_example(serialized, feature_list_dense_missing_assumed_empty, context_sparse_keys, context_dense_keys, feature_list_sparse_keys, feature_list_dense_keys, context_dense_defaults, debug_name, context_sparse_types=[], feature_list_dense_types=[], context_dense_shapes=[], feature_list_sparse_types=[], feature_list_dense_shapes=[], name=None):
  ...
  if _context.in_graph_mode():
    ...
    _attr_Tcontext_dense, context_dense_defaults = _execute.convert_to_mixed_eager_tensors(context_dense_defaults)
    _attr_Tcontext_dense = [_t.as_datatype_enum for _t in _attr_Tcontext_dense]
    serialized = _tensor.convert_to_eager_tensor(serialized, _dtypes.string)
    feature_list_dense_missing_assumed_empty = _tensor.convert_to_eager_tensor(feature_list_dense_missing_assumed_empty, _dtypes.string)
    context_sparse_keys = _tensor.convert_n_to_eager_tensor(context_sparse_keys, _dtypes.string)
    context_dense_keys = _tensor.convert_n_to_eager_tensor(context_dense_keys, _dtypes.string)
    feature_list_sparse_keys = _tensor.convert_n_to_eager_tensor(feature_list_sparse_keys, _dtypes.string)
    feature_list_dense_keys = _tensor.convert_n_to_eager_tensor(feature_list_dense_keys, _dtypes.string)
    debug_name = _tensor.convert_to_eager_tensor(debug_name, _dtypes.string)
    _inputs_flat = [serialized, feature_list_dense_missing_assumed_empty] + list(context_sparse_keys) + list(context_dense_keys) + list(feature_list_sparse_keys) + list(feature_list_dense_keys) + list(context_dense_defaults) + [debug_name]
    ...
    _result = _execute.execute("ParseSingleSequenceExample",
...
def parse_tensor(serialized, out_type, name=None):
  ...
  if _context.in_graph_mode():
    ...
    _result = _execute.execute("ParseTensor", 1, inputs=_inputs_flat,
                               attrs=_attrs, name=name)
  _result = _execute.record_gradient(
...
def string_to_number(string_tensor, out_type=_dtypes.float32, name=None):
  ...
  if _context.in_graph_mode():
    ...
    string_tensor = _tensor.convert_to_eager_tensor(string_tensor, _dtypes.string)
    _inputs_flat = [string_tensor]
    _attrs = ("out_type", out_type)
    _result = _execute.execute("StringToNumber", 1, inputs=_inputs_flat,
                               attrs=_attrs, name=name)
  _result = _execute.record_gradient(
