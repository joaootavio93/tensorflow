...
def sdca_fprint(input, name=None):
  ...
  if _context.in_graph_mode():
    ...
    _inputs_flat = [input]
    _attrs = None
    _result = _execute.execute("SdcaFprint", 1, inputs=_inputs_flat,
...
def sdca_optimizer(sparse_example_indices, sparse_feature_indices, sparse_feature_values, dense_features, example_weights, example_labels, sparse_indices, sparse_weights, dense_weights, example_state_data, loss_type, l1, l2, num_loss_partitions, num_inner_iterations, adaptative=False, name=None):
  ...
  if _context.in_graph_mode():
    ...
    sparse_example_indices = _tensor.convert_n_to_eager_tensor(sparse_example_indices, _dtypes.int64)
    sparse_feature_indices = _tensor.convert_n_to_eager_tensor(sparse_feature_indices, _dtypes.int64)
    sparse_feature_values = _tensor.convert_n_to_eager_tensor(sparse_feature_values, _dtypes.float32)
    dense_features = _tensor.convert_n_to_eager_tensor(dense_features, _dtypes.float32)
    example_weights = _tensor.convert_to_eager_tensor(example_weights, _dtypes.float32)
    example_labels = _tensor.convert_to_eager_tensor(example_labels, _dtypes.float32)
    sparse_indices = _tensor.convert_n_to_eager_tensor(sparse_indices, _dtypes.int64)
    sparse_weights = _tensor.convert_n_to_eager_tensor(sparse_weights, _dtypes.float32)
    dense_weights = _tensor.convert_n_to_eager_tensor(dense_weights, _dtypes.float32)
    example_state_data = _tensor.convert_to_eager_tensor(example_state_data, _dtypes.float32)
    _inputs_flat = list(sparse_example_indices) + list(sparse_feature_indices) + list(sparse_feature_values) + list(dense_features) + [example_weights, example_labels] + list(sparse_indices) + list(sparse_weights) + list(dense_weights) + [example_state_data]
    ...
    _result = _execute.execute("SdcaOptimizer", _attr_num_sparse_features +
...
def sdca_shrink_l1(weights, l1, l2, name=None):
  ...
  if _context.in_graph_mode():
    ...
    raise RuntimeError(
        "sdca_shrink_l1 op does not support eager execution.")
  return _result
